...
- Do not enable browser globals. This matches the WordPress default — `@wordpress/eslint-plugin` deliberately does not enable the `browser` environment (the old v6 config added `env: { browser: true }`). In `.js` / `.jsx` files, bare DOM-only globals (`MutationObserver`, `getComputedStyle`, `IntersectionObserver`, `ResizeObserver`, …) are no longer predefined; add them per project via `languageOptions.globals` or access them through `window.`. `.ts` / `.tsx` files are unaffected (`no-undef` is off for TypeScript). Removes the `globals` dependency.
